...
def process_zipfile_zipped(zipf, max_bytes, exts, found, recursion_depth,
                           max_recursion):
    """
    ...
    for zi in zipf.infolist():
        ...
        if recursion_depth < max_recursion and is_archive_name(name):
            process_bytes_as_archive(b, name, max_bytes, exts, found,
...
def process_tarfile_obj(tarf, max_bytes, exts, found, recursion_depth,
                        max_recursion):
...
def process_bytes_as_archive(b,
                             name,
                             max_bytes,
                             exts,
                             found,
        ...
        if lname.endswith(('.zip', '.whl')):
            try:
                with zipfile.ZipFile(bio) as zf:
                    process_zipfile_zipped(zf, max_bytes, exts, found,
        ...
        if any(
            ...
            try:
                ...
                with tarfile.open(fileobj=bio, mode='r:*') as tf:
                    process_tarfile_obj(tf, max_bytes, exts, found,
                                        recursion_depth, max_recursion)
...
def process_path(path, max_bytes, exts, found, recursion_limit=3):
    """
        ...
        if any(lname.endswith(suf) for suf in ('.zip', '.whl')):
            try:
                with zipfile.ZipFile(path) as zf:
                    process_zipfile_zipped(zf, max_bytes, exts, found, 0,
                                           recursion_limit)
                return
            except zipfile.BadZipFile:
        ...
        if any(
            ...
            try:
                with tarfile.open(path, mode='r:*') as tf:
                    process_tarfile_obj(tf, max_bytes, exts, found, 0,
                                        recursion_limit)
                return
...
def main():
    ...
    exts = (
        {e.strip().lower()
         ...
         if e.strip()} if args.extensions else None)
