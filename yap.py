...
def format_single_file(file: Path, args) -> bool:
    ...
    try:
        ...
        if args.remove_all_unused_imports:
            ...
            code = autoflake.fix_code(code, remove_all_unused_imports=True, remove_unused_variables=True)
...
def main() -> None:
    ...
    p.add_argument("-r", "--remove-all-unused-imports", action="store_true", help="Autoflake cleanup")
    ...
    if args.files:
        ...
        files = [
            f
            for f in Path(".").rglob("*")
            if f.is_file() and not any(part in IGNORED_DIRS for part in f.parts) and is_python_file(f)
        ]
