...
def remove_comments_and_docstrings(source: str) -> str:
    ...
    for tok in tokenize.generate_tokens(io_obj.readline):
        ...
        if token_type == tokenize.COMMENT:
            continue
        elif token_type == tokenize.NL and prev_toktype == tokenize.NEWLINE:
            continue
        else:
...
def main():
    parser = argparse.ArgumentParser(
        description="Clean Python files by removing comments and docstrings.")
    ...
    with ThreadPoolExecutor() as executor:
        futures = {
            executor.submit(process_file, path, args.inplace): path
            for path in targets
        }
